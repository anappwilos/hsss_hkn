...
# Revert constants
old_constants = '''const adminPanelTabs = getElement<HTMLElement>('#admin-sidebar-nav');
...
new_constants = '''const adminPanelTabs = getElement<HTMLElement>('#admin-panel-tabs');
...
# Revert renderAdminPanel and remove renderAdminDashboard
import re
render_pattern = r'function renderAdminPanel\(\): void \{.*?\}\n\nfunction renderAdminDashboard\(\): void \{.*?\}'
# Actually it's easier to find the whole block and replace it.

old_render_and_dashboard = '''function renderAdminPanel(): void {
...
if old_render_and_dashboard in content:
    content = content.replace(old_render_and_dashboard, new_render)
else:
    print("Error: old render/dashboard not found")
    sys.exit(1)

with open(file_path, 'w') as f:
    f.write(content)
print("Successfully reverted render logic")
